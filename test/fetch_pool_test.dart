...
void main() {
  group('Fetch Pool', () {
    final imageGifBytes = base64Decode(
    ...
    test('filenameFromUrl', () {
      ...
      expect(
          FetchPool.filenameFromUrl(
              url, FetchPoolFileNamingStrategy.basenameWithQueryParams),
          'img.png');
      expect(
          FetchPool.filenameFromUrl(urlWithQuery,
              FetchPoolFileNamingStrategy.basenameWithQueryParams),
          'img_a_123_b_456.png');
      expect(
          FetchPool.filenameFromUrl(urlWithoutExtension,
              FetchPoolFileNamingStrategy.basenameWithQueryParams),
          'img');
      expect(
          FetchPool.filenameFromUrl(urlWithoutExtensionWithQuery,
              FetchPoolFileNamingStrategy.basenameWithQueryParams),
          'img_a_123_b_456');
      ...
      expect(
          FetchPool.filenameFromUrl(
              url, FetchPoolFileNamingStrategy.base64EncodedUrl),
          'aHR0cHM6Ly90ZXN0LmNvbS9pbWcucG5n');
      expect(
          FetchPool.filenameFromUrl(
              urlWithQuery, FetchPoolFileNamingStrategy.base64EncodedUrl),
          'aHR0cHM6Ly90ZXN0LmNvbS9pbWcucG5nP2E9MTIzJmI9NDU2');
      expect(
          FetchPool.filenameFromUrl(urlWithoutExtension,
              FetchPoolFileNamingStrategy.base64EncodedUrl),
          'aHR0cHM6Ly90ZXN0LmNvbS9pbWc=');
      expect(
          FetchPool.filenameFromUrl(urlWithoutExtensionWithQuery,
              FetchPoolFileNamingStrategy.base64EncodedUrl),
          'aHR0cHM6Ly90ZXN0LmNvbS9pbWc_YT0xMjMmYj00NTY=');
    });
    ...
    test('Single Image', () async {
      ...
      List<FileSystemEntity> entries =
          Directory(destinationDir).listSync(recursive: false).toList();
    ...
    test('Local filenames with basename fileNamingStrategy (default)',
        () async {
      final urls = List.generate(
          10, (index) => 'http://example.com/image$index.png?v=123&p=456');
      final fetchPool = FetchPool(
          maxConcurrent: 5, urls: urls, destinationDirectory: destinationDir);
      fetchPool.client = MockClient((request) async {
        return Response.bytes(imageGifBytes, 200);
      ...
      for (var urlString in urls) {
        ...
        final expectedDestinationPath =
            p.join(destinationDir, p.basename(url.path));
    ...
    test('Local filenames with basenameWithQueryParams fileNamingStrategy',
        () async {
      final urls = List.generate(
          10, (index) => 'http://example.com/image$index.png?v=123&p=456');
      final fetchPool = FetchPool(
      ...
      for (var urlString in urls) {
        ...
        final expectedDestinationPath = p.join(
            destinationDir, '${basenameWithoutExt}_v_123_p_456$extension');
        final result = results[urlString];
    ...
    test('Local filenames with base64EncodedUrl fileNamingStrategy', () async {
      ...
      for (var urlString in urls) {
        ...
        final expectedDestinationPath =
            p.join(destinationDir, base64EncodedUrl);
        final result = results[urlString];
    ...
    test('Ensure maxConcurrent is greater than 0', () async {
      expect(
          () =>
              FetchPool(maxConcurrent: 0, urls: [], destinationDirectory: '.'),
          throwsArgumentError);
      expect(
          () =>
              FetchPool(maxConcurrent: -1, urls: [], destinationDirectory: '.'),
          throwsArgumentError);
    });
    ...
    test('Ensure destinationDirectory is not empty', () async {
      ...
      expect(
          () => FetchPool(
              maxConcurrent: 1, urls: [], destinationDirectory: '       '),
          throwsArgumentError);
    });
